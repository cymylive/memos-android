...
- **大文件支持** — 附件走本地流式存储，上传与备份/恢复均无实际大小限制（支持 4GB+ 视频，ZIP64 备份格式）
...
> 备份使用 ZIP64 格式，单个备份包可远超 4GB，含大视频在内的一切数据均可完整备份与恢复（恢复时请保证设备有足够剩余空间）。
...
- [架构与开发文档](docs/ARCHITECTURE.md) — 技术架构、大文件上传管线、备份格式、本地构建方式
