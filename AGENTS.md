...
- Android: `mobile/` is a Go library (bound via gomobile) embedding the server on `0.0.0.0:8081`; `android/` is a Kotlin WebView shell with backup/restore.
...
- Backend must compile with `CGO_ENABLED=0` (Android/mobile constraint); do not introduce cgo dependencies.
- `mobile/` runs the server on `0.0.0.0` (LAN sharing) — never change it to loopback only. gomobile-exported functions take only simple types (string/int/bool).
- APK data layout: everything (SQLite `memos_prod.db`, attachments, caches) lives under the data dir; backup/restore zips that whole dir, so never relocate data outside it.
...
# Android APK (LAN-shared, offline single-binary)
cd web && pnpm install && pnpm release   # MUST build SPA first (go:embed dist)
gomobile init
gomobile bind -target=android/arm64 -androidapi 26 -o android/app/libs/mobile.aar ./mobile
cd android && ./gradlew assembleDebug   # produces app-debug.apk (arm64)
```
...
| `mobile/` | gomobile-exported Go library: `StartServer/StopServer/IsRunning`; binds `0.0.0.0:8081`, all data under passed dataDir |
| `android/` | Kotlin WebView shell + foreground service + backup/restore (SAF + zip); depends on `app/libs/mobile.aar` |
